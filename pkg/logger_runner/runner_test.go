package logger_runner

import (
	"context"
	"fmt"
	"os"
	"testing"
	"time"

	"github.com/castmetal/cloudwalk-tests-quake-game-log/pkg/reports"
)

func TestKillData(t *testing.T) {
	testCase := []struct {
		description string
		input       string
		expected    *PlayerKilledData
	}{
		{
			description: "testing if kill string log has the same data  collected by GetPlayerKilledData method",
			input:       "  1:26 Kill: 1022 4 22: <world> killed Zeh by MOD_TRIGGER_HURT",
			expected: &PlayerKilledData{
				KillerPlayer: "<world>",
				DeadPlayer:   "Zeh",
				Mod:          reports.MOD_TRIGGER_HURT,
			},
		},
		{
			description: "testing if kill string log with a killer long string has the same data  collected by GetPlayerKilledData method",
			input:       "  1:26 Kill: 1022 4 22: Dono da Bola killed Isgalamido by MOD_ROCKET_SPLASH",
			expected: &PlayerKilledData{
				KillerPlayer: "Dono da Bola",
				DeadPlayer:   "Isgalamido",
				Mod:          reports.MOD_ROCKET_SPLASH,
			},
		},
		{
			description: "testing if kill string log with a killer long string and dead player long string has the same data  collected by GetPlayerKilledData method",
			input:       "  1:26 Kill: 1022 4 22: Assasinu Credi Assasinu Credi Assasinu Credi killed Dono da Bola Dono da Bola Dono da Bola by MOD_FALLING",
			expected: &PlayerKilledData{
				KillerPlayer: "Assasinu Credi Assasinu Credi Assasinu Credi",
				DeadPlayer:   "Dono da Bola Dono da Bola Dono da Bola",
				Mod:          reports.MOD_FALLING,
			},
		},
	}

	for _, tc := range testCase {
		t.Run(tc.description, func(t *testing.T) {
			logger := NewLoggerRunner()

			killedPlayerData := logger.getPlayerKilledData(tc.input)
			if tc.expected.Mod != killedPlayerData.Mod {
				t.Errorf("invalid_mod_error() : expected %v, got => %v", tc.expected.Mod, killedPlayerData.Mod)
			}

			if tc.expected.DeadPlayer != killedPlayerData.DeadPlayer {
				t.Errorf("invalid_dead_player() : expected %s, got => %s", tc.expected.DeadPlayer, killedPlayerData.DeadPlayer)
			}

			if tc.expected.KillerPlayer != killedPlayerData.KillerPlayer {
				t.Errorf("invalid_killer_player() : expected %s, got => %s", tc.expected.KillerPlayer, killedPlayerData.KillerPlayer)
			}
		})
	}
}

func createTempFile(pathDir, logFile string) (*os.File, error) {
	// Create a temporary file using ioutil.TempFile
	file, err := os.CreateTemp(pathDir, logFile)
	if err != nil {
		return nil, err
	}

	return file, nil
}

func TestRun(t *testing.T) {
	pathDir := "../../inputs"
	logFile := "testfile_.txt"

	tempFile, err := createTempFile(pathDir, logFile)
	if err != nil {
		t.Errorf("Error creating temporary file: %v", err)
		return
	}

	defer tempFile.Close()

	defer func() {
		err := os.Remove(tempFile.Name())
		if err != nil {
			return
		}
	}()

	logTestFile := `
	0:00 ------------------------------------------------------------
	0:00 InitGame: \sv_floodProtect\1\sv_maxPing\0\sv_minPing\0\sv_maxRate\10000\sv_minRate\0\sv_hostname\Code Miner Server\g_gametype\0\sv_privateClients\2\sv_maxclients\16\sv_allowDownload\0\dmflags\0\fraglimit\20\timelimit\15\g_maxGameClients\0\capturelimit\8\version\ioq3 1.36 linux-x86_64 Apr 12 2009\protocol\68\mapname\q3dm17\gamename\baseq3\g_needpass\0
   15:00 Exit: Timelimit hit.
   20:34 ClientConnect: 2
   20:34 ClientUserinfoChanged: 2 n\Isgalamido\t\0\model\xian/default\hmodel\xian/default\g_redteam\\g_blueteam\\c1\4\c2\5\hc\100\w\0\l\0\tt\0\tl\0
   20:37 ClientUserinfoChanged: 2 n\Isgalamido\t\0\model\uriel/zael\hmodel\uriel/zael\g_redteam\\g_blueteam\\c1\5\c2\5\hc\100\w\0\l\0\tt\0\tl\0
   21:07 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   22:06 Kill: 2 3 7: Isgalamido killed Mocinha by MOD_ROCKET_SPLASH
   22:06 Kill: 2 3 7: Mocinha killed Isgalamido by MOD_TRIGGER_HURT
   20:37 ClientBegin: 2
   20:37 ShutdownGame:
   20:37 ------------------------------------------------------------
   20:37 ------------------------------------------------------------
   20:37 InitGame: \sv_floodProtect\1\sv_maxPing\0\sv_minPing\0\sv_maxRate\10000\sv_minRate\0\sv_hostname\Code Miner Server\g_gametype\0\sv_privateClients\2\sv_maxclients\16\sv_allowDownload\0\bot_minplayers\0\dmflags\0\fraglimit\20\timelimit\15\g_maxGameClients\0\capturelimit\8\version\ioq3 1.36 linux-x86_64 Apr 12 2009\protocol\68\mapname\q3dm17\gamename\baseq3\g_needpass\0
   20:38 ClientConnect: 2
   20:38 ClientUserinfoChanged: 2 n\Isgalamido\t\0\model\uriel/zael\hmodel\uriel/zael\g_redteam\\g_blueteam\\c1\5\c2\5\hc\100\w\0\l\0\tt\0\tl\0
   20:38 ClientBegin: 2
   20:40 Item: 2 weapon_rocketlauncher
   20:40 Item: 2 ammo_rockets
   20:42 Item: 2 item_armor_body
   20:54 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   20:59 Item: 2 weapon_rocketlauncher
   21:04 Item: 2 ammo_shells
   21:07 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   21:10 ClientDisconnect: 2
   21:15 ClientConnect: 2
   21:15 ClientUserinfoChanged: 2 n\Isgalamido\t\0\model\uriel/zael\hmodel\uriel/zael\g_redteam\\g_blueteam\\c1\5\c2\5\hc\100\w\0\l\0\tt\0\tl\0
   21:17 ClientUserinfoChanged: 2 n\Isgalamido\t\0\model\uriel/zael\hmodel\uriel/zael\g_redteam\\g_blueteam\\c1\5\c2\5\hc\100\w\0\l\0\tt\0\tl\0
   21:17 ClientBegin: 2
   21:18 Item: 2 weapon_rocketlauncher
   21:21 Item: 2 item_armor_body
   21:32 Item: 2 item_health_large
   21:33 Item: 2 weapon_rocketlauncher
   21:34 Item: 2 ammo_rockets
   21:42 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   21:49 Item: 2 weapon_rocketlauncher
   21:51 ClientConnect: 3
   21:51 ClientUserinfoChanged: 3 n\Dono da Bola\t\0\model\sarge/krusade\hmodel\sarge/krusade\g_redteam\\g_blueteam\\c1\5\c2\5\hc\95\w\0\l\0\tt\0\tl\0
   21:53 ClientUserinfoChanged: 3 n\Mocinha\t\0\model\sarge\hmodel\sarge\g_redteam\\g_blueteam\\c1\4\c2\5\hc\95\w\0\l\0\tt\0\tl\0
   21:53 ClientBegin: 3
   22:04 Item: 2 weapon_rocketlauncher
   22:04 Item: 2 ammo_rockets
   22:06 Kill: 2 3 7: Isgalamido killed Mocinha by MOD_ROCKET_SPLASH
   22:11 Item: 2 item_quad
   22:11 ClientDisconnect: 3
   22:18 Kill: 2 2 7: Isgalamido killed Isgalamido by MOD_ROCKET_SPLASH
   22:26 Item: 2 weapon_rocketlauncher
   22:27 Item: 2 ammo_rockets
   22:40 Kill: 2 2 7: Isgalamido killed Isgalamido by MOD_ROCKET_SPLASH
   22:43 Item: 2 weapon_rocketlauncher
   22:45 Item: 2 item_armor_body
   23:06 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   23:09 Item: 2 weapon_rocketlauncher
   23:10 Item: 2 ammo_rockets
   23:25 Item: 2 item_health_large
   23:30 Item: 2 item_health_large
   23:32 Item: 2 weapon_rocketlauncher
   23:35 Item: 2 item_armor_body
   23:36 Item: 2 ammo_rockets
   23:37 Item: 2 weapon_rocketlauncher
   23:40 Item: 2 item_armor_shard
   23:40 Item: 2 item_armor_shard
   23:40 Item: 2 item_armor_shard
   23:40 Item: 2 item_armor_combat
   23:43 Item: 2 weapon_rocketlauncher
   23:57 Item: 2 weapon_shotgun
   23:58 Item: 2 ammo_shells
   24:13 Item: 2 item_armor_shard
   24:13 Item: 2 item_armor_shard
   24:13 Item: 2 item_armor_shard
   24:13 Item: 2 item_armor_combat
   24:16 Item: 2 item_health_large
   24:18 Item: 2 ammo_rockets
   24:19 Item: 2 weapon_rocketlauncher
   24:22 Item: 2 item_armor_body
   24:24 Item: 2 ammo_rockets
   24:24 Item: 2 weapon_rocketlauncher
   24:36 Item: 2 item_health_large
   24:43 Item: 2 item_health_mega
   25:05 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   25:09 Item: 2 weapon_rocketlauncher
   25:09 Item: 2 ammo_rockets
   25:11 Item: 2 item_armor_body
   25:18 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   25:21 Item: 2 weapon_rocketlauncher
   25:22 Item: 2 ammo_rockets
   25:34 Item: 2 weapon_rocketlauncher
   25:41 Kill: 1022 2 19: <world> killed Isgalamido by MOD_FALLING
   25:50 Item: 2 item_armor_combat
   25:52 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT
   25:54 Item: 2 ammo_rockets
   25:55 Item: 2 weapon_rocketlauncher
   25:55 Item: 2 weapon_rocketlauncher
   25:59 Item: 2 item_armor_shard
   25:59 Item: 2 item_armor_shard
   26:05 Item: 2 item_armor_shard
   26:05 Item: 2 item_armor_shard
   26:05 Item: 2 item_armor_shard
   26:09 Item: 2 weapon_rocketlauncher
   26  0:00 ------------------------------------------------------------
   `

	content := []byte(logTestFile)
	_, err = tempFile.Write(content)
	if err != nil {
		t.Errorf("Error writing to temporary file => %v", err)
		return
	}

	type Reports struct {
		DeathsReport  reports.DeathsReport
		GroupedReport reports.GroupedReport
	}

	deathsReports1 := make(reports.DeathsReport, 0)
	mapKillMeansReport1 := make(map[reports.KillMods]int32, 0)
	mapKillMeansReport1[reports.MOD_TRIGGER_HURT] = 2
	mapKillMeansReport1[reports.MOD_ROCKET_SPLASH] = 1
	deathsReports1["game-1"] = reports.Game{
		KillsByMeans: mapKillMeansReport1,
	}
	groupedReports1 := make(reports.GroupedReport, 0)
	groupedReports1["game-1"] = reports.GroupedInformationReport{
		TotalKills: 3,
		Players:    []string{"Isgalamido", "Mocinha"},
		Kills: map[string]int32{
			"Isgalamido": 0,
			"Mocinha":    1,
		},
	}

	deathsReports2 := make(reports.DeathsReport, 0)
	mapKillMeansReport2 := make(map[reports.KillMods]int32, 0)
	mapKillMeansReport2[reports.MOD_TRIGGER_HURT] = 7
	mapKillMeansReport2[reports.MOD_FALLING] = 1
	mapKillMeansReport2[reports.MOD_ROCKET_SPLASH] = 3
	deathsReports2["game-2"] = reports.Game{
		KillsByMeans: mapKillMeansReport2,
	}
	groupedReports2 := make(reports.GroupedReport, 0)
	groupedReports2["game-2"] = reports.GroupedInformationReport{
		TotalKills: 11,
		Players:    []string{"Isgalamido", "Mocinha"},
		Kills: map[string]int32{
			"Isgalamido": -5,
			"Mocinha":    0,
		},
	}

	testCase := []struct {
		description     string
		input           string
		expectedGame    string
		expectedReports Reports
	}{
		{
			description:  "Testing runner with a log example for a game-1",
			input:        tempFile.Name(),
			expectedGame: fmt.Sprintf("game-%d", 1),
			expectedReports: Reports{
				DeathsReport:  deathsReports1,
				GroupedReport: groupedReports1,
			},
		},
		{
			description:  "Testing runner with a log example for a game-2",
			input:        tempFile.Name(),
			expectedGame: fmt.Sprintf("game-%d", 2),
			expectedReports: Reports{
				DeathsReport:  deathsReports2,
				GroupedReport: groupedReports2,
			},
		},
	}
	var killMod reports.KillMods
	for _, tc := range testCase {
		t.Run(tc.description, func(t *testing.T) {
			ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
			defer cancel()

			logger := NewLoggerRunner()
			runnerResponse, err := logger.Run(ctx, tc.input)
			if err != nil {
				t.Errorf("runner error => %v", err)
			}

			groupReportGame := *runnerResponse.GroupedReport

			if groupReportGame[tc.expectedGame].TotalKills != tc.expectedReports.GroupedReport[tc.expectedGame].TotalKills {
				t.Errorf("invalid_total_kills() : expected %d, got => %d", groupReportGame[tc.expectedGame].TotalKills, tc.expectedReports.GroupedReport[tc.expectedGame].TotalKills)
			}

			for playerName, kills := range tc.expectedReports.GroupedReport[tc.expectedGame].Kills {
				if groupReportGame[tc.expectedGame].Kills[playerName] != kills {
					t.Errorf("%s Kills is different for %s: expected %d, got => %d", playerName, tc.expectedGame, kills, groupReportGame[tc.expectedGame].Kills[playerName])
				}
			}

			deathReportGame := *runnerResponse.DeathsReport

			for mod, totalKills := range deathReportGame[tc.expectedGame].KillsByMeans {
				if tc.expectedReports.DeathsReport[tc.expectedGame].KillsByMeans[mod] != totalKills {
					killMod = mod
					killModStr := killMod.GetStrModByType()
					t.Errorf("Total kills by mod %s is different: expected %d, got => %d", killModStr, tc.expectedReports.DeathsReport[tc.expectedGame].KillsByMeans[mod], totalKills)
				}
			}
		})
	}
}
